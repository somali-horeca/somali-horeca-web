      ...
      <section className="px-6 py-16">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <span
                className={`inline-block rounded-sm px-2 py-1 font-mono text-[10px] font-bold uppercase ${TIER_STYLE[business.membershipTier]}`}
              >
          ...
          <p className="mt-2 font-mono text-sm text-ink/50">📍 {business.location}</p>
          ...
          <div className="mt-8 grid grid-cols-3 gap-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="flex h-24 items-center justify-center rounded-md bg-gradient-to-br from-blue to-ink font-mono text-xs text-paper/50"
              >
                Image {i}
              </div>
            ))}
          ...
          <div className="mt-10">
            <h2 className="font-serif text-xl font-semibold">Products &amp; Services</h2>
            <ul className="mt-3 flex flex-wrap gap-2">
              {business.productsAndServices.map((p) => (
                <li
                  key={p}
                  className="rounded-full border border-ink/15 bg-paper-dark px-3 py-1 text-sm text-ink/75"
                >
                  {p}
                </li>
              ))}
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-2">
            <div>
              ...
              <ul className="mt-3 space-y-2 text-sm text-ink/75">
                ...
                <li>✉️ {business.contact.email}</li>
                {business.website && (
                  <li>
                    🌐{" "}
            ...
            <div>
              ...
              {business.brochureUrl ? (
                <a
                  ...
                  className="mt-3 inline-block rounded-sm bg-ink px-5 py-2.5 text-sm font-semibold text-paper hover:opacity-90"
                >
                  Download PDF
                </a>
              ) : (
          ...
          <div className="mt-10 rounded-md border border-ink/15 bg-paper-dark p-6">
            ...
            <form className="mt-4 grid gap-3">
              <input
                ...
                className="rounded-sm border border-ink/15 bg-paper px-4 py-2.5 text-sm"
              />
              <input
                type="text"
              ...
              <textarea
                ...
                className="min-h-24 rounded-sm border border-ink/15 bg-paper px-4 py-2.5 text-sm"
              />
              <button
                type="submit"
                className="rounded-sm bg-gold px-5 py-2.5 text-sm font-semibold text-ink hover:opacity-90"
              >
                Send Inquiry
              </button>
              <p className="text-xs text-ink/50">
                Demo form — not yet connected. Will route to the business&apos;s contact or a
                platform inbox once wired up.
              </p>
            </form>
          </div>
        </div>
